        ...
        if (SGCore::SG::SenseGlove::GetSenseGlove(testGlove)) //retrieves the first Sense Glove it can find. Returns true if one can be found
        {
            ...
            while (true) {
                ...
                if (testGlove.GetGlovePose(glovePose))
                {
                    ...
                    //float dThumbIndex = tipPositions[0].DistTo(tipPositions[2]); //calculates the distance between thumb (0) and index finger (1), in mm.
                ...
                std::this_thread::sleep_for(std::chrono::milliseconds(500)); //wait for 10ms.
            }
        }
        else {
